...
const PostDetail = ({ post }) => {
  ...
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch(type) {
      ...
      case 'heading-five':
        return <h5 key={index} className="text-sm font-semibold mb-4">
          {modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)};
        </h5>
      case 'heading-six':
        return <h6 key={index} className="text-xs font-semibold mb-4">
          {modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)};
        </h6>
      case 'paragraph':
        ...
        <div className='px-4 lg:px-0'>
          <div className='flex items-center mb-8 w-full'>
            <div className="flex items-center justify-center mb-4 lg:mb-0 w-full lg:w-auto mr-8">
              ...
              />
              <p className="inline align-middle text-gray-700 ml-2 font-medium text-lg">{post.author.authorName}</p>
            </div>
